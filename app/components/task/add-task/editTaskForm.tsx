...
export default function EditTaskForm({ task, setOpen }: any) {
  ...
  const onSubmit = async () => {
    ...
    try {
      await axios
        .put(`${baseUrl}/users/todos/${task._id}?userId=${user._id}`, data, {
          headers: {
  ...
  return (
    <form>
      <Stack spacing={2}>
        ...
        <Button onClick={(e) =>{ e.preventDefault(); onSubmit();}}>Submit</Button>
      </Stack>
    </form>
  );
}
